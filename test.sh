...
test_netbox_unit_tests() {
  ...
  $doco run --rm netbox python manage.py makemigrations netbox_device_lifecycle_mgmt --check
  $doco run --rm netbox python manage.py test netbox_device_lifecycle_mgmt
}
...
test_cleanup() {
  ...
  $doco down -v
  $doco rm -fsv
  docker image rm docker.io/library/netbox-device-lifecycle-mgmt-netbox || echo ''
}
